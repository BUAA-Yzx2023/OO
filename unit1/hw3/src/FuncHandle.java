...
public class FuncHandle {
    ...
    private static String garg1;
    private static String garg2;
    private static String harg1;
    private static String harg2;
    ...
    private static Map<String, String> normalfunctionMap;
    ...
    public static void Scanf(Scanner scanner) {
        ...
        scanner.nextLine();
        if (n > 0) {
            normalfunctionMap = parseNormalDefinition(n, scanner);
        }
        n = scanner.nextInt();
        scanner.nextLine();
        if (n > 0) {
    ...
    private static Map<String, String> parseNormalDefinition(int n, Scanner scanner) {
        Map<String, String> functionMap = new HashMap<>();
        for (int i = 0; i < n; i++) {
            String line = scanner.nextLine().trim();
            // 去掉空白符
            line = line.replaceAll("\\s", "");
            functionMap.put(String.valueOf(line.charAt(0)), extractExpression(line));
            if (line.charAt(0) == 'g') {
                garg1 = String.valueOf(line.charAt(2));
                garg2 = String.valueOf(line.charAt(4));
            } else {
                harg1 = String.valueOf(line.charAt(2));
                harg2 = String.valueOf(line.charAt(4));
            }
        }
        return functionMap;
    }
    ...
    public static String computeFn(String name, String x, String y) {
        String result = normalfunctionMap.get(name);
        if (name.equals("g")) {
            result = result.replace(garg1, "#").replace(garg2, "$");
        } else {
            result = result.replace(harg1, "#").replace(harg2, "$");
        }
        return result.replace("#", "(" + x + ")").replace("$", "(" + y + ")");
    }

    public static String computeFn(String name, String x) {
        String result = normalfunctionMap.get(name);
        if (name.equals("g")) {
            return result.replace(garg1, "(" + x + ")");
        } else {
            return result.replace(harg1, "(" + x + ")");
        }
    ...
    private static String[] extractPara2Array(String line) {
        int top = -1;
        // 初始化数组，含4个StringBuilder
        StringBuilder[] resultArray = new StringBuilder[4];
        ...
        return new String[]{resultArray[0].toString(),
