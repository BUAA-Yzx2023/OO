...
class MainGame():
    ...
    def blitGroupResult(self):
        for i in range(self.groupNum):
            ...
            button_text = font.render(self.groupResults[i][0].name, True, (0, 0, 0))  # 渲染文本
    ...
    def blitWaitCompile(self):
        if self.waitCom == 1 :
            ...
            button_rect = button_text.get_rect(topleft=(160 + 25, 250 + 45))
            self.window.blit(button_text, button_rect)
    ...
    def blitWaitRun(self):
        ...
        if wait < 6 and self.noTest==0 :
            ...
            button_rect = button_text.get_rect(topleft=(300 + 25, 600 + 45))
            self.window.blit(button_text, button_rect)
...
def getCompute(input_string):
    bin_dir = 'Compute_class'  # 替换为您希望编译后的.class文件存放的目录
    # 运行MainClass
    run_command = ['java', '-cp', bin_dir, 'MainClass']
...
def check(answer,output):
    ...
    lenAns = len(answer)
    print(answer,output)
    x = round(random.uniform(2.1,5.9),3)
    answer = f'{x}\n{answer}'
    output = f'{x}\n{output}'
    ansCom = getCompute(answer)
    outCom = getCompute(output)
    print(ansCom,outCom)
    isEqual = (ansCom[0:12]==outCom[0:12])
    if (isEqual):
        pro = lenOut/lenAns
        return 1,pro
    else:
